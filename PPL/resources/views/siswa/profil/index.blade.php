        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
                <nav class="flex items-center gap-2 text-xs text-slate-500 mb-1">
                    <a href="{{ route('siswa.dashboard') }}" class="hover:text-slate-900 transition-colors">Dashboard</a>
                    <span>/</span>
                    <span class="text-slate-700 font-medium">Profil Siswa</span>
                </nav>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Profil & Pengaturan Akun</h1>
                <p class="text-sm text-slate-500 mt-0.5">Kelola identitas pribadi, kontak, dan keamanan kata sandi Anda</p>
            </div>
            <div>
                <a href="{{ route('siswa.dashboard') }}" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs font-semibold shadow-xs transition-colors">
            ...
            <div class="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div class="flex flex-col items-center w-full md:w-64 shrink-0 pb-6 md:pb-0 md:border-r md:border-slate-100 md:pr-8 text-center">
                    <div class="relative group cursor-pointer" id="avatarWrapper">
                        <img id="profileImage"
                            src="{{ $siswa->foto_siswa ? asset('images/siswa/' . $siswa->foto_siswa) : 'https://ui-avatars.com/api/?name='.urlencode($siswa->nama_siswa ?? 'Siswa').'&background=06466C&color=ffffff&size=256' }}"
                            alt="Foto Profil"
                    ...
                    <h2 class="text-base font-bold text-slate-900 mt-4">{{ $siswa->nama_siswa ?? 'Nama Siswa' }}</h2>
                    <p class="text-xs font-mono text-slate-500 mt-0.5">NISN: {{ $siswa->nisn ?? '-' }}</p>
                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-[#06466C]/10 text-[#06466C] mt-2.5">
                        {{ ucfirst($siswa->role_siswa ?? 'Siswa') }}
                    </span>
                </div>
                ...
                <div class="flex-1 w-full">
                    ...
                    <p class="text-xs text-slate-500 mb-5">Perbarui username, email, nomor kontak, atau ubah kata sandi akun Anda</p>

                    <form method="POST" action="{{ route('siswaprofil.update') }}" id="updateForm" class="space-y-5" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div class="sm:col-span-2">
                                <label for="nomor_wa_siswa" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Nomor WhatsApp / Telepon</label>
