<x-admin-layout>
    <div class="p-6 max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Data Siswa</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui profil dan kredensial akun peserta didik</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 sm:p-8">
            <form action="{{ route('siswa.update', $siswa->id_siswa) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username"
                            ...
                            value="{{ old('username', $siswa->username) }}" placeholder="Username akun siswa" required>
                        @error('username')
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_siswa" id="nama_siswa"
                            class="w-full text-sm rounded-lg border border-slate-300 px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#06466C]/20 focus:border-[#06466C] transition-colors"
                            value="{{ old('nama_siswa', $siswa->nama_siswa) }}" placeholder="Nama lengkap siswa" required>
                        @error('nama_siswa')
                            <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nisn" id="nisn"
                            ...
                            value="{{ old('nisn', $siswa->nisn) }}" placeholder="10 digit NISN" required>
                        @error('nisn')
                            <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="tgl_lahir_siswa" id="tgl_lahir_siswa"
                            class="w-full text-sm rounded-lg border border-slate-300 px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#06466C]/20 focus:border-[#06466C] transition-colors"
                            value="{{ old('tgl_lahir_siswa', $siswa->tgl_lahir_siswa) }}" required>
                    ...
                    <div>
                        ...
                        <select id="role_siswa" name="role_siswa"
                            ...
                            <option value="siswa" {{ old('role_siswa', $siswa->role_siswa) == 'siswa' ? 'selected' : '' }}>Siswa Reguler</option>
                            <option value="pengurus" {{ old('role_siswa', $siswa->role_siswa) == 'pengurus' ? 'selected' : '' }}>Pengurus Ekstrakurikuler</option>
                        </select>
                        @error('role_siswa')
                            <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                            class="w-full text-sm rounded-lg border border-slate-300 px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#06466C]/20 focus:border-[#06466C] transition-colors"
                            value="{{ old('email', $siswa->email) }}" placeholder="siswa@sekolah.sch.id" required>
                        @error('email')
                            <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_wa_siswa" id="nomor_wa_siswa"
                            class="w-full text-sm rounded-lg border border-slate-300 px-3.5 py-2.5 font-mono focus:outline-none focus:ring-2 focus:ring-[#06466C]/20 focus:border-[#06466C] transition-colors"
                            value="{{ old('nomor_wa_siswa', $siswa->nomor_wa_siswa) }}" placeholder="08xxxxxxxxxx">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 mb-2">
                            <img class="h-12 w-12 rounded-full object-cover border border-slate-200" 
                                src="{{ $siswa->foto_siswa ? asset('storage/' . $siswa->foto_siswa) : 'https://ui-avatars.com/api/?name='.urlencode($siswa->nama_siswa).'&background=06466C&color=ffffff' }}" 
                                alt="{{ $siswa->nama_siswa }}">
                            <span class="text-xs text-slate-500">Unggah file baru untuk mengganti foto saat ini</span>
                        </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="alamat_siswa" id="alamat_siswa" rows="3"
                            class="w-full text-sm rounded-lg border border-slate-300 px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#06466C]/20 focus:border-[#06466C] transition-colors"
                            placeholder="Alamat domisili tempat tinggal siswa">{{ old('alamat_siswa', $siswa->alamat_siswa) }}</textarea>
                        @error('alamat_siswa')
                            <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
