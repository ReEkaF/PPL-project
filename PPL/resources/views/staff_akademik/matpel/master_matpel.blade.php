<x-staffakademik-layout>
    <div class="space-y-6 pb-10">
                ...
                <nav class="flex" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 text-xs">
                        ...
                        <li class="flex items-center text-slate-400">
                            ...
                            <span class="text-slate-800 font-medium">Master Mata Pelajaran</span>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Master Mata Pelajaran</h1>
                <p class="text-xs text-slate-500">
                    Kelola data kurikulum, daftar mata pelajaran, dan deskripsi kompetensi pembelajaran.
                </p>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-slate-50/40">
                <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Daftar Mata Pelajaran</h2>
                <form action="{{ route('staff_akademik.mata-pelajaran.index') }}" method="GET" class="flex items-center gap-2">
                    ...
                    @if(request('search'))
                        <a href="{{ route('staff_akademik.mata-pelajaran.index') }}"
                        ...
                        @forelse($mataPelajaran as $index => $pelajaran)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-5 py-3.5 text-center font-mono text-slate-400">
                                    {{ $index + 1 + ($mataPelajaran->currentPage() - 1) * $mataPelajaran->perPage() }}
                                </td>
                                <td class="px-5 py-3.5 font-semibold text-slate-800">
                                    <div class="inline-flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-lg bg-brand-50 border border-brand-100/60 text-brand-800 flex items-center justify-center font-bold text-xs">
                                            <i class="fa-solid fa-book-open text-[11px]"></i>
                                        </div>
                                        <span>{{ $pelajaran->nama_matpel }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-3.5 text-slate-500 max-w-xs truncate">
                                    {{ $pelajaran->deskripsi_matpel ?: '-' }}
                                </td>
                                <td class="px-5 py-3.5 text-right whitespace-nowrap">
                                    <div class="inline-flex items-center gap-1.5 justify-end">
                                        <button type="button" onclick="openEditModal('{{ $pelajaran->id_matpel }}', '{{ addslashes($pelajaran->nama_matpel) }}', '{{ addslashes($pelajaran->deskripsi_matpel) }}')"
                                        ...
                                        <form method="POST" action="{{ route('staff_akademik.mata-pelajaran.destroy', $pelajaran->id_matpel) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus mata pelajaran ini?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-5 py-12 text-center text-slate-400">
                                    ...
                                    <span>Belum ada data mata pelajaran yang terdaftar atau sesuai pencarian.</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="crud-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div class="flex items-center justify-between p-4 md:p-5 border-b border-slate-100 bg-slate-50/50">
                <div>
                    <h3 class="text-sm font-bold text-slate-900">Tambah Mata Pelajaran</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Daftarkan mata pelajaran kurikulum baru.</p>
                </div>
            ...
            <form method="POST" action="{{ route('staff_akademik.mata-pelajaran.store') }}" class="p-5 space-y-4">
                @csrf
                <div>
                    <label for="create_nama_matpel" class="block text-xs font-semibold text-slate-700 mb-1.5">Nama Mata Pelajaran</label>
                    <input type="text" name="nama_matpel" id="create_nama_matpel"
                ...
                <div>
                    <label for="create_deskripsi_matpel" class="block text-xs font-semibold text-slate-700 mb-1.5">Deskripsi Singkat</label>
                    <textarea name="deskripsi_matpel" id="create_deskripsi_matpel" rows="3"
                        placeholder="Deskripsi ringkas mata pelajaran (opsional)..."
                        class="block w-full text-xs text-slate-800 bg-white border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-colors"></textarea>
                </div>
    ...
    <div id="edit-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div class="flex items-center justify-between p-4 md:p-5 border-b border-slate-100 bg-slate-50/50">
                <div>
                    <h3 class="text-sm font-bold text-slate-900">Perbarui Mata Pelajaran</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Ubah nama atau deskripsi mata pelajaran.</p>
                </div>
            ...
            <form method="POST" id="edit-form" class="p-5 space-y-4">
                ...
                @method('PUT')
                <div>
                    <label for="edit_nama_matpel" class="block text-xs font-semibold text-slate-700 mb-1.5">Nama Mata Pelajaran</label>
                    <input type="text" name="nama_matpel" id="edit_nama_matpel"
                        class="block w-full text-xs text-slate-800 bg-white border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-colors"
                        required>
                </div>
                <div>
                    <label for="edit_deskripsi_matpel" class="block text-xs font-semibold text-slate-700 mb-1.5">Deskripsi Singkat</label>
                    <textarea name="deskripsi_matpel" id="edit_deskripsi_matpel" rows="3"
                        class="block w-full text-xs text-slate-800 bg-white border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-colors"></textarea>
                </div>
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button type="button" onclick="closeEditModal()"
    ...
    <script>
        ...
        function openEditModal(id, name, desc) {
            ...
            document.getElementById('edit_deskripsi_matpel').value = desc;
